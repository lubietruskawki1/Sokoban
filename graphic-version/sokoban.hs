...
startScreen :: Picture
startScreen = translated 0 3 (scaled 3 3 (lettering "Sokoban!"))
              & translated 0 (-4) etap4

etap4 :: Picture
etap4 = pictureOfBools (map isClosedAndSane allMazes)
  where
    allMazes = appendList mazes badMazes
    isClosedAndSane :: Maze -> Bool
    isClosedAndSane = \m -> isClosed m && isSane m
...
handleEvent (KeyPress key) state
  | key == "N"                   = goToNextLevelIfPossible state
...
endScreen :: Picture
endScreen = scaled 1.3 1.3 (lettering "Wszystkie poziomy ukończone!")

winScreen :: State -> Picture
winScreen state = lettering (fromString ("Poziom ukończony, liczba ruchów: "
                                         ++ show (stMove state)))

-- draws player, reachable boxes and the rest of maze separately
drawBoard :: State -> Picture
drawBoard state = atCoord (stPlayer state) (player2 (stDir state))
...
player2 direction =
  case direction of
    R -> rotated (-pi/2) player1
    U -> player1
    L -> rotated (pi/2) player1
    D -> rotated pi player1

player1 :: Picture
player1 = hair & shirt & arms
  where
    hair = solidCircle 0.25
    shirt = colored black (rectangle 1 0.25)
            & colored shirtColor (solidRectangle 1 0.25)
    arms = translated (-0.4) 0.25 arm & translated 0.4 0.25 arm
      where arm = colored black (rectangle 0.2 0.5)
                  & colored skinColor (solidRectangle 0.2 0.5)
...
wall = bricks
       & colored brickOutlineColor squareOutline
       & colored brickColor square
  where bricks = brick
                 & translated (1/4) (1/3) brick
                 & translated (1/4) (-1/3) brick
                 & translated (-1/4) (1/3) brick
                 & translated (-1/4) (-1/3) brick
          where brick = colored brickOutlineColor (rectangle (1/2) (1/3))
                
ground = colored groundColor square

storage = colored storageColor (solidCircle 0.15)
          & colored groundColor square
          
box = boxTemplate boxColor

-- box placed in the correct position
correctBox = boxTemplate correctBoxColor
          
boxTemplate :: Color -> Picture
boxTemplate color = boxPattern
                    & colored boxOutlineColor squareOutline
                    & colored color square
  where boxPattern = colored boxOutlineColor (rectangle 0.7 0.7)
                     & clipped 0.7 0.7 (rotated (pi/4) plank
                                        & rotated (-pi/4) plank)
          where plank = colored boxOutlineColor (rectangle 0.15 1)

square, squareOutline :: Picture
square = solidRectangle 1 1
squareOutline = rectangle 1 1
...
skinColor, shirtColor :: Color
skinColor = colorFromRGBParameters 255 255 133
shirtColor = colorFromRGBParameters 218 218 218

brickColor, brickOutlineColor :: Color
brickColor = colorFromRGBParameters 161 149 85
brickOutlineColor = colorFromRGBParameters 91 85 48

groundColor :: Color
groundColor = colorFromRGBParameters 222 214 174

storageColor:: Color
storageColor = colorFromRGBParameters 214 149 133

boxColor, boxOutlineColor, correctBoxColor :: Color
boxColor = colorFromRGBParameters 255 182 80
boxOutlineColor = colorFromRGBParameters 54 31 0
correctBoxColor = colorFromRGBParameters 186 101 12

-- given RGB parameters, returns a suitable color
colorFromRGBParameters :: Double -> Double -> Double -> Color
colorFromRGBParameters r g b = RGB (r / 255) (g / 255) (b / 255)
...
andList :: [Bool] -> Bool
andList list = foldList allAreTrue True list
  where allAreTrue currentBool trueBefore = currentBool && trueBefore
